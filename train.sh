...
save_dir="retrain_savedir"

tmp_save_dir="retrain_tmp"
tsb_dir="tsb_dir"
...
CUDA_VISIBLE_DEVICES="0" python $(which unicore-train) $data_path --user-dir ./unimol --train-subset train --valid-subset valid \
       --num-workers 8 --ddp-backend=c10d \
       --task tta --loss in_batch_softmax --arch tta  \
       --max-pocket-atoms 256 \
       --optimizer adam --adam-betas "(0.9, 0.999)" --adam-eps 1e-8 --clip-norm 1.0 \
       --lr-scheduler polynomial_decay --lr $lr --tta-lr 1e-3 --tta-lr-p 1e-4 --warmup-ratio $warmup --max-epoch $epoch --batch-size $batch_size --batch-size-valid $batch_size_valid \
       ...
       --save-dir $save_dir --tmp-save-dir $tmp_save_dir --keep-last-epochs 20 \
